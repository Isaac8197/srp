...
// Variables PID
float kp = 2.5, ki = 0.06, kd = 0.8;  // Ajusta estos valores según la respuesta deseada
...
void Leer_Termocupla()
{
    ...
    if (HAL_GetTick() - last_update_time >= 1000) {
        ...
        Mostrar_Pantalla(); // Llama a la pantalla solo cuando se actualiza la temperatura
    }
}

/* Función para leer el encoder y ajustar la temperatura de corte */
void Leer_Encoder()
{
    ...
    // Limitar el rango de set_temperature
    if (set_temperature < 20) set_temperature = 20;
    else if (set_temperature > 200) set_temperature = 200;
}

/* Función para el control PID y ajuste de PWM */
void Control_PID()
{
    float error = set_temperature - temperatura;
    pid_integral += error;
    float pid_derivative = error - pid_last_error;
    pid_output = kp * error + ki * pid_integral + kd * pid_derivative;
    pid_last_error = error;

    // Limitar el valor de pid_output para el PWM
    if (pid_output > 255) pid_output = 255;
    else if (pid_output < 0) pid_output = 0;

    // Ajuste del PWM en PB0
    __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_3, (uint32_t)pid_output);

    // Control de PA3 según la temperatura
    if (temperatura >= set_temperature) {
...
static void MX_TIM3_Init(void)
{
  ...
  htim3.Instance = TIM3;
  htim3.Init.Prescaler = 31500; 		//3150 = 100ms
